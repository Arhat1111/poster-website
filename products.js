...
function categoryName(id){return(siteData.categories||[]).find(c=>c.id===id)?.name||id||'InkWave';}
...
function renderCatalog(){
  const q=searchQuery.trim().toLowerCase(),all=siteData.products||[],allList=all.filter(p=>(activeCategory==='all'||p.category===activeCategory)&&`${p.name} ${p.description||''} ${categoryName(p.category)}`.toLowerCase().includes(q)),list=allList.slice(0,shownCount);
  $('#productsGrid').innerHTML=list.map(p=>{const sizes=productSizes(p),lowest=sizes.length?Math.min(...sizes.map(k=>productPrice(p,k))):0,desc=(p.description||'300 GSM matte laminated poster from the InkWaves collection.').trim();return `<article class="product product-minimal product-catalog-card"><a class="product-image" href="product.html?id=${encodeURIComponent(p.id)}" aria-label="View ${esc(p.name)}"><img referrerpolicy="no-referrer" src="${esc(p.image)}" alt="${esc(p.name)}" loading="lazy" />${p.badge?`<span class="product-badge">${esc(p.badge)}</span>`:''}</a><div class="product-meta"><span class="product-theme">${esc(categoryName(p.category))}</span><h3><a href="product.html?id=${encodeURIComponent(p.id)}">${esc(p.name)}</a></h3><p class="product-card-description">${esc(desc)}</p><div class="product-card-price-row"><div><small>FROM</small><strong>${formatINR(lowest)}</strong></div><label class="product-card-size"><span>SIZE</span><select data-card-size="${esc(p.id)}">${sizes.map(k=>`<option value="${k}">${k} · ${formatINR(productPrice(p,k))}</option>`).join('')}</select></label></div><button class="product-card-add" type="button" data-card-add="${esc(p.id)}" ${sizes.length?'':'disabled'}>${sizes.length?'ADD TO BAG':'UNAVAILABLE'}</button><a class="product-card-details" href="product.html?id=${encodeURIComponent(p.id)}">View details →</a></div></article>`}).join('');
  $('#productsCount').textContent=`Showing ${list.length} of ${allList.length} ${allList.length===1?'poster':'posters'}${activeCategory!=='all'?` in ${categoryName(activeCategory)}`:''}`;$('#productsEmpty').hidden=allList.length>0;const more=$('#productsLoadMore');if(more)more.hidden=list.length>=allList.length;wireImageFallbacks($('#productsGrid'));
}
...
$('#productsCategorySelect').addEventListener('change',e=>{activeCategory=e.target.value;shownCount=48;const url=activeCategory==='all'?'products.html':`products.html?category=${encodeURIComponent(activeCategory)}`;history.replaceState(null,'',url);renderCatalog();});
$('#productsSearch').addEventListener('input',e=>{searchQuery=e.target.value;shownCount=48;renderCatalog();});
$('#productsLoadMore')?.addEventListener('click',()=>{shownCount+=48;renderCatalog();});
loadProductsData().catch(e=>{console.error(e);$('#productsCount').textContent='Unable to load products.';});
