...
function renderReviews(){const reviews=siteData.reviews||[],track=$('#reviewsTrack');if(!track)return;const stars=n=>'★'.repeat(Math.max(1,Math.min(5,Number(n)||5)))+'☆'.repeat(5-Math.max(1,Math.min(5,Number(n)||5)));track.innerHTML=reviews.map((r,i)=>{const avatar=r.image?`<img class="review-avatar" src="${escapeHTML(r.image)}" alt="${escapeHTML(r.name||'Customer')}">`:`<span class="review-avatar review-avatar-fallback">${escapeHTML((r.name||'C').trim().charAt(0).toUpperCase())}</span>`;return`<article class="review-card"><div class="review-card-top"><span class="review-stars">${stars(r.rating)}</span><span class="review-index">${String(i+1).padStart(2,'0')}</span></div><blockquote>“${escapeHTML(r.text||'')}”</blockquote><div class="review-person">${avatar}<div class="review-person-copy"><strong>${escapeHTML(r.name||'Customer')}</strong><span>${escapeHTML(r.location||'')}</span></div></div>${r.product?`<small>${escapeHTML(r.product)}</small>`:''}</article>`}).join('');const avg=reviews.length?reviews.reduce((a,r)=>a+(Number(r.rating)||5),0)/reviews.length:5;if($('#reviewAverage'))$('#reviewAverage').textContent=avg.toFixed(1);if($('#reviewCount'))$('#reviewCount').textContent=`${reviews.length} ${reviews.length===1?'review':'reviews'}`}
