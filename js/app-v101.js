...
const mediaRequests={product:new Set(),category:new Set(),reviews:false};
async function hydrateProductMedia(items,scope=document){if(!window.INKWAVES_FIREBASE||typeof window.INKWAVES_FIREBASE.loadProductMedia!=='function')return;const list=(items||[]).filter(p=>p&&!p.image&&!mediaRequests.product.has(String(p.id)));if(!list.length)return;list.forEach(p=>mediaRequests.product.add(String(p.id)));try{const media=await window.INKWAVES_FIREBASE.loadProductMedia(list.map(p=>p.id));list.forEach(p=>{const src=media[p.id];if(!src)return;p.image=src;scope.querySelectorAll(`[data-product-media=\"${CSS.escape(String(p.id))}\"]`).forEach(img=>{img.src=optimizedImageUrl(src,720);img.classList.remove('remote-image-failed')})})}catch(e){console.warn('InkWaves product media load skipped:',e)}}
async function hydrateCategoryMedia(items,scope=document){if(!window.INKWAVES_FIREBASE||typeof window.INKWAVES_FIREBASE.loadCategoryMedia!=='function')return;const list=(items||[]).filter(c=>c&&!c.image&&!mediaRequests.category.has(String(c.id)));if(!list.length)return;list.forEach(c=>mediaRequests.category.add(String(c.id)));try{const media=await window.INKWAVES_FIREBASE.loadCategoryMedia(list.map(c=>c.id));list.forEach(c=>{const src=media[c.id];if(!src)return;c.image=src;scope.querySelectorAll(`[data-category-media=\"${CSS.escape(String(c.id))}\"]`).forEach(img=>{img.src=optimizedImageUrl(src,700);img.classList.remove('remote-image-failed')})})}catch(e){console.warn('InkWaves category media load skipped:',e)}}
async function hydrateReviewMedia(){if(mediaRequests.reviews||!window.INKWAVES_FIREBASE||typeof window.INKWAVES_FIREBASE.loadReviewMedia!=='function')return;mediaRequests.reviews=true;try{const reviews=siteData.reviews||[],media=await window.INKWAVES_FIREBASE.loadReviewMedia(reviews.map(r=>r.id));reviews.forEach(r=>{const row=media[r.id]||{};r.image=row.avatar||r.image||'';r.productImages=Array.isArray(row.productImages)?row.productImages:(r.productImages||[])});renderReviews()}catch(e){console.warn('InkWaves review media load skipped:',e)}}
function setupReviewMediaLoading(){const section=$('#reviews');if(!section)return;if(!('IntersectionObserver'in window)){setTimeout(hydrateReviewMedia,1200);return}const io=new IntersectionObserver(entries=>{if(entries.some(e=>e.isIntersecting)){io.disconnect();hydrateReviewMedia()}},{rootMargin:'500px 0px'});io.observe(section)}
...
async function loadSite(){
  ...
  products=shuffleProducts(siteData.products||[]);applySiteContent();renderSlider();renderOffers();renderCategories();renderFilters();renderProducts();renderReviews();setupReviewMediaLoading();renderCart();renderPolaroidBuilder();bindEvents();
...
function renderSlider(){const wall=$('#polaroidWall');if(!wall)return;const base=(siteData.heroSlides||[]).slice(0,5).map((s,i)=>({image:optimizedImageUrl(s.image,650),label:s.label||`InkWaves ${i+1}`,caption:s.caption||'Poster drop'}));if(!base.length)return;wall.innerHTML=Array.from({length:3},(_,lane)=>{const ordered=base.map((_,i)=>base[(i+lane)%base.length]);const cards=ordered.map((item,i)=>`<article class="polaroid-card polaroid-tilt-${(i+lane)%4}"><div class="polaroid-photo"><img src="${escapeHTML(item.image)}" alt="${escapeHTML(item.label)} poster" loading="lazy" decoding="async"></div><div class="polaroid-copy"><strong>${escapeHTML(item.label)}</strong><span>${escapeHTML(item.caption)}</span></div></article>`).join('');return`<div class="polaroid-lane lane-${lane+1}"><div class="polaroid-run">${cards}${cards}</div></div>`}).join('');wireImageFallbacks(wall)}
function renderReviews(){const reviews=siteData.reviews||[],track=$('#reviewsTrack');if(!track)return;const stars=n=>'★'.repeat(Math.max(1,Math.min(5,Number(n)||5)))+'☆'.repeat(5-Math.max(1,Math.min(5,Number(n)||5)));track.innerHTML=reviews.map((r,i)=>{const personName=r.name||'Customer';const avatar=r.image?`<button type="button" class="review-avatar-button" data-review-photo="${escapeHTML(r.image)}" data-review-photo-alt="${escapeHTML(personName)}"><img class="review-avatar" src="${escapeHTML(r.image)}" alt="${escapeHTML(personName)}" loading="lazy" decoding="async"></button>`:`<span class="review-avatar review-avatar-fallback">${escapeHTML((personName||'C').trim().charAt(0).toUpperCase())}</span>`;const photos=(Array.isArray(r.productImages)?r.productImages:[]).slice(0,4);const gallery=photos.length?`<div class="review-product-gallery ${photos.length===1?'single':''}">${photos.map((src,j)=>`<button type="button" class="review-product-photo" data-review-photo="${escapeHTML(src)}" data-review-photo-alt="Received product photo ${j+1}" aria-label="Open received product photo ${j+1}"><img src="${escapeHTML(src)}" alt="Received InkWaves product photo ${j+1}" loading="lazy" decoding="async"></button>`).join('')}</div>`:'';const title=String(r.title||'').trim();const reviewText=String(r.text||'').trim();return`<article class="review-card"><div class="review-card-top"><span class="review-stars">${stars(r.rating)}</span><span class="review-index">${String(i+1).padStart(2,'0')}</span></div>${gallery}${title?`<h3 class="review-title">${escapeHTML(title)}</h3>`:''}${reviewText?`<p class="review-copy">${escapeHTML(reviewText)}</p>`:''}<div class="review-person">${avatar}<div class="review-person-copy"><strong>${escapeHTML(personName)}</strong><span>${escapeHTML(r.location||'')}</span></div></div>${r.product?`<small>${escapeHTML(r.product)}</small>`:''}</article>`}).join('');const avg=reviews.length?reviews.reduce((a,r)=>a+(Number(r.rating)||5),0)/reviews.length:5;if($('#reviewAverage'))$('#reviewAverage').textContent=avg.toFixed(1);if($('#reviewCount'))$('#reviewCount').textContent=`${reviews.length} ${reviews.length===1?'review':'reviews'}`}

function openReviewLightbox(src,alt='Review photo'){if(!src)return;let lightbox=$('#reviewLightbox');if(!lightbox){lightbox=document.createElement('div');lightbox.id='reviewLightbox';lightbox.className='review-lightbox';lightbox.hidden=true;lightbox.innerHTML=`<div class="review-lightbox-backdrop" data-review-lightbox-close></div><figure class="review-lightbox-frame"><button type="button" class="review-lightbox-close" data-review-lightbox-close aria-label="Close photo">×</button><img alt=""><figcaption></figcaption></figure>`;document.body.appendChild(lightbox)}const img=lightbox.querySelector('img'),caption=lightbox.querySelector('figcaption');img.src=src;img.alt=alt||'Review photo';caption.textContent=alt||'';lightbox.hidden=false;document.documentElement.classList.add('review-lightbox-open')}
function closeReviewLightbox(){const lightbox=$('#reviewLightbox');if(!lightbox)return;lightbox.hidden=true;lightbox.querySelector('img').removeAttribute('src');document.documentElement.classList.remove('review-lightbox-open')}
...
function renderCategories(){const cats=siteData.categories||[],section=$('#categories');if(section)section.hidden=cats.length===0;$('#categoryGrid').innerHTML=cats.map((c,i)=>{const src=optimizedImageUrl(c.image||'assets/image-fallback.svg',700);return`<a class="category-tile" style="--cat-accent:${escapeHTML(c.accent||'#eee')}" href="products.html?category=${encodeURIComponent(c.id)}"><span class="category-number">${String(i+1).padStart(2,'0')}</span><span class="category-art"><img data-category-media="${escapeHTML(c.id)}" src="${escapeHTML(src)}" alt="${escapeHTML(c.name)} poster category" loading="lazy" decoding="async"></span><span class="category-info"><strong>${escapeHTML(c.name)}</strong><small>${escapeHTML(c.subtitle)}</small><b>SHOP CATEGORY →</b></span></a>`}).join('');$('#categoryBar').innerHTML=`<a href="products.html">Shop all</a>${cats.map(c=>`<a href="products.html?category=${encodeURIComponent(c.id)}">${escapeHTML(c.name)}</a>`).join('')}<button data-custom-product>Custom</button>`;wireImageFallbacks($('#categoryGrid'));const track=$('#categoryGrid');hydrateCategoryMedia(cats.slice(0,4),track);if(track&&'IntersectionObserver'in window){const io=new IntersectionObserver(entries=>{entries.forEach(entry=>{if(!entry.isIntersecting)return;const id=entry.target.dataset.categoryMedia,cat=cats.find(c=>String(c.id)===String(id));if(cat)hydrateCategoryMedia([cat],track);io.unobserve(entry.target)})},{root:track,rootMargin:'0px 220px'});track.querySelectorAll('[data-category-media]').forEach(img=>{const cat=cats.find(c=>String(c.id)===String(img.dataset.categoryMedia));if(cat&&!cat.image)io.observe(img)})}requestAnimationFrame(()=>{if(track){track.scrollLeft=0;updateCategorySliderUI()}})}
...
function renderProducts(){const q=state.query.trim().toLowerCase(),filtered=products.filter(p=>(state.filter==='all'||p.category===state.filter)&&`${p.name} ${p.category} ${p.description}`.toLowerCase().includes(q)),visible=filtered.slice(0,(state.filter==='all'&&!q)?homeProductLimit():20);els.productGrid.innerHTML=visible.map(p=>{const sizes=productSizes(p),lowest=sizes.length?Math.min(...sizes.map(k=>productPrice(p,k))):0,src=optimizedImageUrl(p.image||'assets/image-fallback.svg',700);return`<article class="product home-product-card"><a class="product-image" href="product.html?id=${encodeURIComponent(p.id)}" aria-label="View ${escapeHTML(p.name)}"><img data-product-media="${escapeHTML(p.id)}" referrerpolicy="no-referrer" src="${escapeHTML(src)}" alt="${escapeHTML(p.name)}" loading="lazy" decoding="async">${p.badge?`<span class="product-badge">${escapeHTML(p.badge)}</span>`:''}</a><div class="product-meta"><span class="product-theme">${escapeHTML(categoryName(p.category))}</span><h3><a href="product.html?id=${encodeURIComponent(p.id)}">${escapeHTML(p.name)}</a></h3><p class="product-card-description">${escapeHTML(p.description||'300 GSM matte laminated poster.')}</p><div class="home-product-bottom"><strong>From ${formatINR(lowest)}</strong><button type="button" data-quick-view="${escapeHTML(p.id)}" ${sizes.length?'':'disabled'}>${sizes.length?'CHOOSE SIZE + ADD':'UNAVAILABLE'}</button></div></div></article>`}).join('');els.emptyState.hidden=filtered.length>0;wireImageFallbacks(els.productGrid);hydrateProductMedia(visible,els.productGrid)}
...
async function startSecureRazorpayPayment(customer,totals,paymentWindow){
  ...
  try{
    ...
    if(message.toLowerCase().includes('missing or insufficient permissions')||String(err?.code||'').includes('permission-denied')){
      throw new Error('Order saving is blocked by Firebase rules. Publish the Firestore rules before taking payments.');
    }
    throw err;
  }
