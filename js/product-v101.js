...
function optimizedImageUrl(src,width=850){const value=String(src||'');return value.includes('drive.google.com/thumbnail')?value.replace(/([?&]sz=)w\d+/,'$1w'+width):value}
const requestedProductMedia=new Set();
async function ensureProductMedia(items){if(!window.INKWAVES_FIREBASE||typeof window.INKWAVES_FIREBASE.loadProductMedia!=='function')return;const missing=(items||[]).filter(p=>p&&!p.image&&!requestedProductMedia.has(String(p.id)));if(!missing.length)return;missing.forEach(p=>requestedProductMedia.add(String(p.id)));const media=await window.INKWAVES_FIREBASE.loadProductMedia(missing.map(p=>p.id));missing.forEach(p=>{if(media[p.id])p.image=media[p.id]})}
...
async function loadData(){
  ...
  try{await ensureProductMedia([product])}catch(e){console.warn('Product artwork could not be loaded:',e)}
...
function renderMockups(){
  ...
  track.innerHTML=`<article class="product-mockup-slide direct-product-slide"><div class="direct-product-art"><img class="mockup-poster-art" referrerpolicy="no-referrer" src="${esc(optimizedImageUrl(product.image||'assets/image-fallback.svg',900))}" alt="${esc(product.name)}" /></div></article>`;
...
function renderRelated(){
  const relatedPool=(siteData.products||[]).filter(p=>p.id!==product.id),related=[...shuffleProducts(relatedPool.filter(p=>p.category===product.category)),...shuffleProducts(relatedPool.filter(p=>p.category!==product.category))].slice(0,3);
  $('#relatedProducts').innerHTML=related.map(p=>{const src=optimizedImageUrl(p.image||'assets/image-fallback.svg',650);return`<article class="related-card"><a class="related-card-image" href="product.html?id=${encodeURIComponent(p.id)}"><img data-related-media="${esc(p.id)}" referrerpolicy="no-referrer" src="${esc(src)}" alt="${esc(p.name)}" loading="lazy" decoding="async"></a><div class="related-card-copy"><span class="related-card-theme">${esc(categoryName(p.category))}</span><h3><a href="product.html?id=${encodeURIComponent(p.id)}">${esc(p.name)}</a></h3><p>From ${formatINR(Math.min(...sizeOrder(p).map(k=>productPrice(p,k)))||0)}</p><a class="related-card-link" href="product.html?id=${encodeURIComponent(p.id)}">View product →</a></div></article>`}).join('');
  wireImageFallbacks($('#relatedProducts'));
  ensureProductMedia(related).then(()=>{related.forEach(p=>{if(!p.image)return;document.querySelectorAll(`[data-related-media=\"${CSS.escape(String(p.id))}\"]`).forEach(img=>img.src=optimizedImageUrl(p.image,650))})}).catch(()=>{});
}
