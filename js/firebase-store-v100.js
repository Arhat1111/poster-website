...
/**
 * Restore the archived InkWaves catalog without overwriting current products.
 * Only IDs missing from the dedicated Firestore collections are created.
 * Existing modern product/category documents always win.
 */
async function restoreArchivedCatalog(archive, options = {}) {
  requireAdmin();

  const archivedProducts = Array.isArray(archive?.products) ? archive.products : [];
  const archivedCategories = Array.isArray(archive?.categories) ? archive.categories : [];
  if (!archivedProducts.length && !archivedCategories.length) {
    throw new Error('The archived catalog file is empty or invalid.');
  }
  ...
  const existingProducts = new Set(productSnap.docs.map(d => String(d.id)));
  const existingCategories = new Set(categorySnap.docs.map(d => String(d.id)));

  const missingCategories = archivedCategories.filter(c => {
    const id = String(c?.id || '').trim();
    return id && !existingCategories.has(id);
  });
  const missingProducts = archivedProducts.filter(p => {
    const id = String(p?.id || '').trim();
    return id && !existingProducts.has(id);
  });

  const total = missingCategories.length + missingProducts.length;
  let completed = 0;
  onProgress({ completed, total, missingProducts: missingProducts.length, missingCategories: missingCategories.length });

  const categoryBaseOrder = categorySnap.size;
  const productBaseOrder = productSnap.size;

  const writeInBatches = async (items, kind, baseOrder) => {
    for (let i = 0; i < items.length; i += 12) {
      const batch = items.slice(i, i + 12);
      await Promise.all(batch.map(async (item, localIndex) => {
        const id = String(item.id).trim();
        const payload = clean({ ...item, order: baseOrder + i + localIndex, restoredFromArchive: true });
        delete payload.id;
        await setDoc(doc(db, kind, id), payload);
      }));
  ...
  // Categories first so restored product filters are immediately valid.
  await writeInBatches(missingCategories, 'categories', categoryBaseOrder);
  await writeInBatches(missingProducts, 'products', productBaseOrder);

  // Force the next dashboard/storefront read to use the newly restored documents.
  lastLoaded = null;
  lastCatalogDocs = { products: [], categories: [] };

  return {
    archivedProducts: archivedProducts.length,
    archivedCategories: archivedCategories.length,
    restoredProducts: missingProducts.length,
    restoredCategories: missingCategories.length,
    skippedProducts: archivedProducts.length - missingProducts.length,
    skippedCategories: archivedCategories.length - missingCategories.length
  };
}
...
window.INKWAVES_FIREBASE = {
  ...
  loadMergedData, saveWholeStore, optimizeCatalogMedia, restoreArchivedCatalog, deleteProduct, deleteCategory,
